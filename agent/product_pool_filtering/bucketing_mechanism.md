...
$$
P_{i,s}^{(\text{return_3y})} \le 0.10
$$

2. **OR 夏普比率分位数处于该策略内最差 10%**

$$
P_{i,s}^{(\text{sharpe_ratio_3y})} \le 0.10
$$

3. **OR（波动率分位数处于最差 / 最高 10%，且收益率低于策略中位数）**

$$
P_{i,s}^{(\text{volatility_3y})} \ge 0.90
\quad \text{AND} \quad
P_{i,s}^{(\text{return_3y})} < 0.50
$$
