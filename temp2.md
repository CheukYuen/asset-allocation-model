下面是 `scripts/run_backtest.py` 的完整 Prompt，你可以直接丢给 Cursor 生成脚本。

这个脚本会做这几件事：

* 从 `../data/` 读 CSV（SAA / 回测收益 / 象限）
* 用 `core.taa_signal_engine` 算 TAA 后的 `final_weights`
* 用 `core.backtest_engine` 进行回测，算年化收益、波动、Sharpe、MDD、Calmar、胜率
* 打印一份“迷你回测报表”

---

### ✅ Prompt：生成 `scripts/run_backtest.py`
...
你可以先用这个 Prompt 让 Cursor 生成 `scripts/run_backtest.py`，跑一下：

```bash
python scripts/run_mock_data.py   # 先生成 CSV
python scripts/run_backtest.py    # 再跑回测报表
````

如果输出的指标你想再细化（比如加 Tracking Error、信息比率、按大类拆解收益贡献），我们可以再往 backtest_engine 和 scripts 层一起加一版 2.0。
